            ...
            {expanded && (
                <div className="px-3.5 pb-3.5 pt-2 border-t border-slate-700/40 space-y-2.5">
                    ...
                    <div className="flex items-center justify-between gap-2 pt-0.5">
                        <a href={v.helpUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[11px] text-slate-500 hover:text-slate-300 transition-colors">
...
function SeoRow({ check }: { check: SeoCheck }) {
    ...
    return (
        <div className={cn(
            "rounded-lg border bg-slate-800/40 p-3.5",
            isPass ? "border-emerald-500/30" : "border-red-500/30 bg-red-500/5"
        )}>
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] bg-slate-900/80 rounded px-2.5 py-2 font-mono text-slate-300 border border-slate-700/50 break-all">
                        {check.actualValue || "No value found"}
                    </div>
                </div>
            </div>
        </div>
                ...
                {status === "complete" ? (
                    <div className="flex bg-slate-900/80 p-1 rounded-lg border border-slate-700/50">
                        <button
                            ...
                            className={cn("flex items-center gap-1.5 px-3 py-1 text-[11px] font-semibold rounded-md transition-colors", view === "a11y" ? "bg-slate-700 text-slate-200 shadow-sm" : "text-slate-500 hover:text-slate-300")}
                        >
                            Accessibility
                            <span className="bg-slate-800 text-slate-400 px-1.5 rounded-full text-[9px]">{violations.length}</span>
                        </button>
                        <button
                            onClick={() => setView("seo")}
                            className={cn("flex items-center gap-1.5 px-3 py-1 text-[11px] font-semibold rounded-md transition-colors", view === "seo" ? "bg-slate-700 text-slate-200 shadow-sm" : "text-slate-500 hover:text-slate-300")}
                        >
                            SEO Health
                            <span className="bg-slate-800 text-slate-400 px-1.5 rounded-full text-[9px]">{seoResults.filter(s => s.status === "fail").length}</span>
                        </button>
                    </div>
                ) : (
                ...
                {status === "idle" && (
                    <div className="flex flex-col items-center justify-center h-full gap-3 py-8 text-center">
                        ...
                        <p className="text-xs text-slate-500 max-w-[200px]">
                            Enter a URL and click <strong className="text-slate-400">Scan</strong> to audit for WCAG and SEO issues.
                        </p>
                    </div>
                )}
